...
\midrule
\texttt{/api/disease/detect/} & Detect disease from uploaded crop image \\
\texttt{/api/disease/train/} & Trigger model training (admin scope) \\
\texttt{/api/weather/\{location\}/} & Retrieve weather and agricultural advice \\
\texttt{/api/market/prices/} & Get current market prices \\
\texttt{/api/market/predict/\{crop\}/} & Forecast short-term price trend \\
\texttt{/api/advisory/chat/} & AI chat advisory endpoint \\
\texttt{/api/rag/search/} & Retrieve relevant knowledge entries \\
\bottomrule
\end{longtable}
...
Validation Accuracy (best\_val\_acc from \texttt{models/checkpoint.pth}) & 85.0565\% \\
Last Validation Accuracy (val\_acc from \texttt{models/checkpoint.pth}) & 85.0565\% \\
Test Accuracy (re-evaluated using saved \texttt{disease\_detector.pth}) & 84.5348\% \\
...
\begin{tabular}{p{5.0cm} p{3.8cm} p{3.8cm}}
\toprule
