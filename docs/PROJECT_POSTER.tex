...
\begin{block}{Objectives (Scoped)}
\begin{itemize}
  \item Build a usable platform with \textbf{secure authentication} and mobile-friendly UI for smallholder workflows.
  \item Implement \textbf{image-based disease detection} as the primary technical scope: \textbf{Tomato, Potato, Pepper Bell} leaves using an exportable ONNX model, with safety checks for unreliable or unsupported inputs.
